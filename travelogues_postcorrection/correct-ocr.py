...
def correct_consonants(txt, textfile):
    ...
    cons_list = [m.start() for m in re.finditer("[bcdfghjklmnpqrstvwxyzß][bcdfghjklmnpqrstvwxyzß][bcdfghjklmnpqrstvwxyzß][bcdfghjklmnpqrstvwxyzß][bcdfghjklmnpqrstvwxyzß]+", txt)]
    if cons_list != []:
        print(len(cons_list))
        print(textfile, "will be corrected now. To correct the shown text, put in the full, corrected text. To skip, type skip. To end correction, type end. (Note: In that case, the file will still be saved normally).")
        #print(cons_list)
