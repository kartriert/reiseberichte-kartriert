...
def correct_consonants(txt, textfile):
    """
    How useful is this? One text has over 1600 found instances that must then all be corrected; however, many texts
    also only have 20-50
    """
    ...
    if letter_list != []:
        print(textfile,
              "will be corrected now. To correct the shown text, put in the full, corrected text. To skip, type s. To "
              "end correction, type e. (Note: In that case, the file will still be saved normally).")
